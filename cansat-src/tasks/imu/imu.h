 ...
 *
 *		This library is based upon the MPU-9250 library by Sparkfun for Arduino
*/
...
//initialization Functions - do not call from FreeRTOS tasks
uint8_t imu_initalizeLib(I2C_HandleTypeDef* interface);
void imu_initAK8963Blocking(float *);
void imu_initMPU9250Blocking();
void imu_MPU9250SelfTest(float * destination);
void imu_calibrateMPU9250Blocking(int32_t * gyroBias, int32_t * accelBias);

//FreeRTOS safe functions
void imu_getMres();
void imu_getGres();
void imu_getAres();
void imu_readAccelData(int16_t *);
void imu_readGyroData(int16_t *);
void imu_readMagData(int16_t *);
...
void imu_writeByte(uint8_t, uint8_t, uint8_t);
uint8_t imu_readByte(uint16_t, uint16_t);
void imu_readBytes(uint8_t, uint8_t, uint8_t, uint8_t *);
uint64_t imu_micros();
